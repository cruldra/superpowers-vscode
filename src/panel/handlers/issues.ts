...
export async function handleGeneratePrDiffSummary(panel: KanbanWebviewPanel, issueNumber: number): Promise<void> {
  try {
    ...
    try {
      ...
      const prompt = `你在一个 VS Code 扩展启动的后台 Claude 会话中工作。\n\n硬性限制：\n- 不要修改代码。\n- 不要创建或切换分支。\n- 不要提交。\n- 不要 push。\n- 不要写任何文件。\n- 只分析当前 PR 的代码变动，并把 Markdown 摘要正文直接输出到 stdout。\n\n任务：\n- 当前工作目录是工单 #${issueNumber} 的 worktree。\n- 分析 PR #${pr} 与主分支的代码差异。\n- 可使用 git diff origin/main...HEAD、tea pulls diff ${pr} 或其他只读 git/tea 命令。\n\n输出必须是合法 Markdown，预览器要能渲染出标题层级与代码块高亮：\n- 第一行是一级标题：# PR #${pr} 代码变更摘要\n- 然后是两个二级标题章节：## 审查重点 和 ## 不太重要的\n- 每个章节里，目录树必须放进围栏代码块（语言标注用 text）：树前一行写三个反引号紧跟 text，树后一行写三个反引号；树内用空格缩进保留 dir/subdir/file 的层级，每个文件行是「文件名 + 若干空格对齐 + 一句目的描述」。\n- 关键：缩进树一定要写在围栏代码块之内，否则 Markdown 预览器会折叠空白、丢掉层级。\n\n格式示例（请照此结构输出，不要照抄内容）：\n\n# PR #123 代码变更摘要\n\n## 审查重点\n\n\`\`\`text\nbackend\n    src/luffy_agent/commerce/catalog\n        repositories.py    新增 reserve_stock 原子预占三态、release_stock 回补\n        service.py         新增 set_stock 补货、release_order_stock 仅真实预占才回补\n\`\`\`\n\n## 不太重要的\n\n\`\`\`text\nbackend\n    src/luffy_agent/core/db\n        repositories.py    BaseRepository.create 入参放宽支持 dict 写入\n    tests/commerce\n        test_catalog.py    覆盖 reserve_stock 三态预占的单元测试\n\`\`\`\n\n要求：\n- 每个新增/修改文件一行，一句话说明新增/修改了什么及其目的。\n- 在代码块内保留原始文件目录结构，使用 ASCII explorer 风格的空格缩进。\n- 按重要性分区：「## 审查重点」只放生产/业务源代码（实际实现逻辑、核心入口、API、数据模型、关键服务等），其余一律放「## 不太重要的」。\n- 测试文件一律归「## 不太重要的」，不得进审查重点。测试文件判定：文件名形如 test_* / *_test.* / *.test.* / *.spec.*，或路径包含 tests/、__tests__/ 目录，或 conftest.py、fixture、mock 文件。\n- 配置、样式、构建产物、纯同步文件仍归「## 不太重要的」。\n- 只输出 Markdown 正文本身，不要写文件，不要任何前导说明/寒暄/结尾总结，第一行就是 # PR #${pr} 代码变更摘要 标题。`
